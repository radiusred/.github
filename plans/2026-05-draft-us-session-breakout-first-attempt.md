...
Equity indices and FX majors do show directional persistence during the US trading day, especially around macro releases. A breakout strategy seemed like the obvious tool: identify the high/low of a reference period, and trade the breakout in the direction of the break.

The logic was sound. Countless papers and practitioners build on similar approaches. What we didn't account for was *which market* we were testing on and *what microstructure* it actually had.

## Testing: healthy trade count, hollow edge

We backtested the strategy on major equity indices and FX pairs using intraday bars, spread-aware fills, and a session window filter. The stop/take-profit sizing was tuned to clear realistic spreads.

Trade volume was reasonable—the system generated thousands of round trips across the instruments. But the win rate told the real story: a significant proportion of all breakout trades reversed back through the entry point and into the stop without ever reaching the take-profit target. In statistical terms, the "breakout" candle was often the entire move. The next few bars retraced it.
...
No amount of parameter tuning was going to fix a market that moves in a way the strategy wasn't designed to handle.
...
- Full technical report: [RAD-96](/RAD/issues/RAD-96)
